...
def melhor_jogada(individuo, estado):
    ...
    maior_valor = np.argmax(valor_das_acoes(individuo, estado))
    return maior_valor
...
def mutacao(individuo):
    ...
    for x in range(3):
        for j in range(10):
            ...
            if prob <= CHANCE_MUT:
                individuo[x][j] = np.random.uniform(-1,1,1)
...
def proxima_geracao(populacao, fitness):
    ...
    while len(proxima_ger) < NUM_INDIVIDUOS:
       ind1, ind2 = random.choices(populacao, weights=fitness, k=2)
